  ...
  document.querySelectorAll("button").forEach((button) => {
    ...
    //Button to add a layer
      ...
      button.onclick = () => {
        ...
        //Add a new layer
        if (newNumber < 4) canvasManager.addLayer(newId);
      };
    }
    ...
    //Changing colour for fill or outline button
      ...
      button.onclick = () => {
        ...
        if (canvasManager.pickingFillColour) {
          ...
          document.getElementById("outlineColUI").style.border =
            "solid black 1px";
        } else {
          button.style.border = "dotted yellow 1px";
...
function getMousePos(canvas, evt) {
  ...
  } //...Or return touch positions if there are no x positions
  else {
    return {
...
function hideLayerFormData() {
  ...
  document.querySelectorAll("label").forEach((label) => {
    if (
      label.htmlFor == "id_layer1_url" ||
      label.htmlFor == "id_layer2_url" ||
      label.htmlFor == "id_layer3_url"
    ) {
