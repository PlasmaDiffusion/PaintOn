...
class CanvasManager {
  ...
  InputDown(canvas, evt, managerObj, ctx) {
    ...
    managerObj.UseDrawingTool(
      ctx,
      0,
      mousePos,
      managerObj.drawingTool,
      managerObj.brushSize,
    ...
    switch (drawingTool) {
      ...
      case 2: //line
        if (mouseState == 0) {
          ...
          this.tempCanvas = ctx.getImageData(
            0,
            0,
            canvasSize.width,
            canvasSize.height
        ...
        if (mouseState == 0) {
          ...
          if (this.fill) {
            ctx.fillRect(
              this.prevMousePos.x,
              this.prevMousePos.y,
              mousePos.x - this.prevMousePos.x,
              mousePos.y - this.prevMousePos.y
            );
          }
          if (this.outline) {
            ctx.rect(
      ...
      case 7: //Colour Picker
        if (mouseState == 2) {
          let newColourArray = ctx.getImageData(mousePos.x, mousePos.y, 1, 1)
            .data;

          //Convert the selected colour to a hexidecimal string
          this.lastClickedColourButton.value =
            "#" +
            this.fullColorHex(
              newColourArray[0],
              newColourArray[1],
  ...
  makeElipse(mousePos, ctx) {
    ...
    ctx.ellipse(
      (this.prevMousePos.x + mousePos.x) / 2,
      (this.prevMousePos.y + mousePos.y) / 2,
      movingToRight
        ? mousePos.x - this.prevMousePos.x
  ...
  ReplaceColours(ctx, mousePos, canvasSize) {
    ...
    for (let j = 0; j <= maxHeightIndex; j += 1) {
      for (
        let i = 0;
        i <= maxWidthIndex;
        i += 4 //+ 4 because 0...3 for colour index values
      ) {
        if (
          this.CheckIfInColourThreshold(
            i,
            j,
            pixels,
            colourToReplace,
  ...
  FillColours(ctx, mousePos, canvasSize) {
    ...
    //Else is the  pixel not equal to the target colour? We don't want to colour over the wrong colour.
      ...
      pixels.data[i + j * maxWidth] != colourToReplace[0] &&
  ...
  CheckIfInColourThreshold(
    ...
    return (
      pixels.data[i + j * canvasSize.width * 4] >=
        colourToReplace[0] - threshold &&
      pixels.data[i + j * canvasSize.width * 4 + 1] >=
        colourToReplace[1] - threshold &&
      pixels.data[i + j * canvasSize.width * 4 + 2] >=
        colourToReplace[2] - threshold &&
      pixels.data[i + j * canvasSize.width * 4 + 3] >=
        colourToReplace[3] - threshold &&
      pixels.data[i + j * canvasSize.width * 4] <=
        colourToReplace[0] + threshold &&
      pixels.data[i + j * canvasSize.width * 4 + 1] <=
        colourToReplace[1] + threshold &&
